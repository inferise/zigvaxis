...
pub fn main(init: std.process.Init) !void {
    ...
    for (0..10) |i| {
        for (lipsum.items, 0..) |paragraph, j| {
            const number = i * 10 + j;
            try model.rows.append(alloc, .{ .idx = number, .text = paragraph });
        }
    }
